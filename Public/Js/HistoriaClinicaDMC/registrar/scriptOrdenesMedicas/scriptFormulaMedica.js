...
$(document).ready(function(){
    ...
    $.ajax({
        ...
        $.each(cmb, function(e,s){
            registroMedicamento.push({descripcion :s.nombre,id: s.idRecurso}); 
        });
    }).fail(function(){
        alert("error")
    });
    ...
    $("#item2").click(function(){
        ...
        $("#btnAgregarDescripcion").click(function(){
            swal({   
                title: "Descripción",   
                text: "<textarea id='txtDescripcionFormulaMedica'>"+descripcionFormulaMedica+"</textarea>",   
                html:true,  
...
function agregarMedicamento(){
    $("#containerMedicamentos").append('<tr><td><button onclick="quitarMedicamento(this)" type="button" class="btn btn-eliminar btnquitarMedicamento" id="btnQuitarMedicacion"><i class="fa fa-times"></i></button></td><td style="width:33%"><div class="frmCont"  style="margin-top:0px"><label></label><div class="frmInput frmInput_select2" style="height:100%"><select id="cmbMedicamento'+(contadorMedicamento+1)+'" name="cmbMedicamento" class="select cmbMedicamentos input_data" onchange="seleccion('+contadorMedicamento+')"></select></div></div></td><td><div class="frmCont"  style="margin-top:0px"><label></label><div class="frmInput"><input type="text" name="txtCantidadMedicamentos'+contadorMedicamento+'" class="txtCantidadMedicamentos input_data" autocomplete="off"></div></div></td><td><div class="frmCont"  style="margin-top:0px"><label></label><div class="frmInput"><input type="text" name="txtDosificacionMedicamento'+contadorMedicamento+'" class="txtDosificacionMedicamento input_data" autocomplete="off"></div></div></td><td><div class="frmCont"  style="margin-top:0px"><label></label><div class="frmInput"><input type="text" name="txtDescripcionMedicamentos'+contadorMedicamento+'"class="txtDescripcionMedicamentos" autocomplete="off"></div></div></td></tr>');
...
function quitarMedicamento(el){
    var posicionIndex = $(".btnquitarMedicamento").index(el);

	$("#containerMedicamentos").children().eq(posicionIndex).remove();
	disabledMedicamento();
	cont--;
}
...
function finalizarFormulaMedica(){
    ...
    if($("#item2").is(":visible")){
        ...
        for(var i=0;i<$(".cmbMedicamentos").length;i++){
            ...
            registrosMedicamentos.push({"id":btoa(id),"cantidad":cantidad,"dosificacion":unescape(encodeURIComponent(dosificacion)),"descripcion":unescape(encodeURIComponent(descripcion))});
        }
