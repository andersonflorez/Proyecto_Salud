...
function consultaCitas(data){
    ...
    if (data != null || data !=0){
        $.each(data.datos,function(s,p){
            if (p.estadoTablaCita == "Proceso") {
                $("#cont").append("<li class='list_item n_dont_grow'>"
                                  ...
                                  +     "<button class='btn btn-consultar' onclick='mostrarCita("+p.idCita+",this)'><i class='fa fa-eye'></i></button>"
                                  +     "<span class='tooltiptext' style='width:80px;'>Detalles</span>"
                                  +   "</div>"
                                  +   "<div class='tooltip separate_right'>"
                                  +     "<button class='btn btn-eliminar' onclick='Cancelar("+p.idCita+")'><i class='fa fa-ban'></i></button>"
                                  +     "<span class='tooltiptext' style='width:80px;'>Cancelar</span>"
                                  +   "</div>"
                                  +   "<div class='tooltip separate_right'>"
                                  +     "<button class='btn btn-modificar' onclick='validaPendiente("+p.idPaciente+","+p.idCitaProgramacion+","+p.idCita+")'><i class='fa fa-history'></i></button>"
        ...
        $.each(data.datos,function(s,p){
            if (p.estadoTablaCita == "Iniciada") {
                $("#cont").append("<li class='list_item n_dont_grow'>"
                                  ...
                                  +   "</div>"
                                  +   "<div class='tooltip separate_right'>"
                                  +     "<button class='btn btn-registrar' onclick='Comfirmar("+p.idPaciente+","+p.idCitaProgramacion+","+p.idCita+")'><i class='fa fa-check'></i></button>"
        ...
        Notificate({
            tipo: 'info',
            titulo: 'No hay citas en la consulta',
...
function Cancelar(idCita){
    ...
    if (id == idCita || localStorage.getItem("Citapendiente") == null) {
        swal({
            title: "Cancelar",
            type: "input",
            ...
            $.ajax({
                ...
                var msj = "No existen multas para asignarle a al paciente, Comúnique con el administrador.";
                swal({title: "Error!",   text: "Motivo: "+msj,   type: "error",   confirmButtonText: "Confirmar",confirmButtonColor: "#E64A65"});
            });
        });
    }else{
        Notificate({
...
$("#btnCerrar").click(function(){
    $("#panelCita").fadeOut(100,function(){
        $("#panelCita").removeClass("xs_flex_col100 sm_flex_col100  md_flex_col60 lg_flex_col65 xl_flex_col75");
        $(".panelAtencionSeleccionado").removeClass("panelAtencionSeleccionado");
        $("#panelDerecho").removeClass("n_flex xs_flex_col100 sm_flex_col100 md_flex_col40 lg_flex_col35 xl_flex_col25 n_flex_col100");
        $("#panelCita").show();
        $("#panelDerecho").hide();
    });
...
function mostrarCita(idCita,el){
    $("#panelCita").fadeOut(100,function(){
        if (!$("#panelCita").hasClass("xs_flex_col100 sm_flex_col100  md_flex_col60 lg_flex_col65 xl_flex_col75")) {
            $("#panelCita").addClass("xs_flex_col100 sm_flex_col100  md_flex_col60 lg_flex_col65 xl_flex_col75");
        }
        if (!$("#panelDerecho").hasClass("n_flex xs_flex_col100 sm_flex_col100 md_flex_col40 lg_flex_col35 xl_flex_col25 n_flex_col100")) {
            $("#panelDerecho").addClass("n_flex xs_flex_col100 sm_flex_col100 md_flex_col40 lg_flex_col35 xl_flex_col25 n_flex_col100");
    ...
    $.ajax({
        ...
        $.each(data,function(s,p){
            ...
            $("#lblCodigoCup").html(p.codigoCUP);
            $("#lblTDocumento").html(p.descripcionTdocumento);
            $("#lblTelefono1").html(p.telefonoFijo1);
            if (p.telefonoFijo2 == null || p.telefonoFijo2 == "") {
              $("#lblTelefono2").html("No tienen");
            }else{
              $("#lblTelefono2").html(p.telefonoFijo2);
            }
            console.log("fin : "+p.telefonoFijo2);
        });
    }).fail(function(){});
}
