...
class mdlRegistrarPersona implements iModel {
  ...
  private $estadoTablaEspecialidad;
  ...
  */
  ...
  function InsertarDatosEspecialidad() {

    $sql   = "CALL spRegistrarPersonaespecialidad(?,?,?)";
    $query = $this->_CONEXION->prepare($sql);
    $query->bindParam(1, $this->idPersona, PDO::PARAM_INT);
    $query->bindParam(2,  $this->idEspecialidad, PDO::PARAM_INT);
    $query->bindParam(3,  $this->estadoTablaEspecialidad);
  ...
  function InsertarDatosUsuario() {
    ...
    $query->bindParam(4,  $this->idRol, PDO::PARAM_INT);
