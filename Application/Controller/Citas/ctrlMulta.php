<?php

/**
* NOMBRE DE LA CLASE: ctrlEnteExterno
* TIPO DE CLASE: Controlador
* DESCRIPCIÓN:
* Controlador de la tabla maestra tbl_enteexterno
* del módulo reporte inicial
*/
class ctrlMulta extends Controller implements iController {

  private $objdiasmultas;

  /**
  * Método constructor()
  * Inicializa el uso de variables de sesión y
  * valida si hay una sesión abierta, sino la hay
  * redirecciona hacia el login de la aplicación:
  */
  public function __construct() {
    //Sesion::init();
    //if (!Sesion::exist()) {
    //header("Location: " . URL . "Usuarios/Usuario");
    //exit();
    //} else {
    $this->objdiasmultas = $this->loadModel('Citas', 'mdlMulta');
    //}
  }

  /**
  * Método Index()
  * Renderiza la página de error debido a que este archivo solo puede
  * ser accedido mediante ajax
  */
  public function Index() {
    header('Location: ' . URL . 'Error/Error');
  }

  // Función para registrar un tipo de evento:
  public function Registrar()
  {
    $valores = json_decode($_POST['Valores']);
    $this->objdiasmultas->SetDiasMulta($valores[0]);
    $FechaSistema =date("Y-m-d");
    $this->objdiasmultas->SetFechaMulta($FechaSistema);
    echo $this->objdiasmultas->RegistrarMulta();
  }

  // Función para modificar un tipo de evento:
  public function Modificar()
  {

    $valores = json_decode($_POST['Valores']);
    $this->objdiasmultas->SetidMulta($valores[0]);
    $this->objdiasmultas->SetDiasMulta($valores[1]);
     $FechaSistema =date("Y-m-d");
    $this->objdiasmultas->SetFechaMulta($FechaSistema);
    echo $this->objdiasmultas->ModificarMulta();
  }

  // Función para cambiar el estado de un tipo de evento:
  public function CambiarEstado()
  {
    $id = $_POST['id'];
    $estado = $_POST['estado'];
    $estado = $estado == 'Activo'? 'Inactivo' : 'Activo';
    $this->objdiasmultas->SetidMulta($id);
    $this->objdiasmultas->SetEstadoMulta($estado);
    echo $this->objdiasmultas->CambiarEstadoMulta();
  }
}

?>
